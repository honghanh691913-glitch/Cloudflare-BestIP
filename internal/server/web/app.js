...
function drawScanDetail(){
  ...
  $('#modalRoot').innerHTML=`<div class="overlay" id="scanOverlay"><div class="sheet"><div class="sheet-head"><h3>${esc(s.name||lineTitle(s))}</h3><button class="sheet-close" data-close>×</button></div><div class="sheet-body" data-ui-key="scan-sheet-body"><div class="scan-head"><div><div class="scan-stage">${esc(st.stage||'等待运行')}</div><div class="help">候选 ${st.candidate_count||s.sample_count||0} · ${fmtFamily(s.family)} ${(s.cfst?.colo||[]).join('/')||'不限地区'}</div></div><button id="healthNowBtn" class="ghost compact">健康检查</button></div><div class="progress-bar"><i style="width:${Math.min(100,Number(p.percent||0))}%"></i></div><div class="scan-stats"><div class="metric"><b>${f.total_candidates||st.candidate_count||0}</b><span>候选</span></div><div class="metric"><b>${f.loss_passed||0}</b><span>初筛</span></div><div class="metric"><b>${tested}</b><span>决赛已测</span></div><div class="metric"><b>${good}</b><span>速度合格</span></div></div>${st.error?`<div class="task-error">${esc(compactError(st.error))}</div>`:''}<div class="mini-label">决赛明细 · 合格置顶并按速度排序</div><div class="result-table" data-ui-key="scan-result-table">${resultRows||'<div class="empty"><b>还没有决赛结果</b>延迟、丢包或地区不合格的 IP 已在前置漏斗淘汰。</div>'}</div><details class="advanced" data-ui-key="scan-live-log" ${st.running?'open':''}><summary>实时日志</summary><pre class="logs" data-ui-key="scan-log-scroll">${esc(logs||'暂无日志')}</pre></details></div><div class="sheet-actions"><button data-close>关闭</button><button class="primary" id="runThisLine">立即严选</button></div></div></div>`;
