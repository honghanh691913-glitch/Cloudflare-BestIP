...
function drawScanDetail(){
  ...
  const rowHtml=(r,kind)=>`<div class="result-row ${kind}"><span class="ip">${esc(r.ip)}</span><span>${esc(r.colo||(!s.cfst?.colo?.length?'不限地区':'未识别'))}</span><span>${Number(r.latency_ms)>0?fmtNum(r.latency_ms,0)+'ms':'—'}</span><span>${r.speed_tested?fmtNum(r.speed_mb,1)+'M':(r.reject_reason?esc(compactError(r.reject_reason)):'待测速')}</span></div>`;
  ...
  const phaseIndex=phase==='probe'?1:phase==='region'?2:phase==='speed'?3:phase==='done'?4:0;
  const flowSteps=['候选生成','延迟 / 丢包','地区识别','速度决赛'].map((x,i)=>`<span class="flow-step ${phaseIndex>i?'done':phaseIndex===i?'active':''}">${phaseIndex>i?'✓ ':''}${x}</span>`).join('');
