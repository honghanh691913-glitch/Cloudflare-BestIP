...
function initConfig(){
  ...
  cfg.real_test_url ||= DEFAULT_REAL_TEST_URL;cfg.real_test_attempts ||=2;cfg.real_test_concurrency ||=5;cfg.real_speed_url ||= cfg.speed_url||DEFAULT_SPEED_URL;cfg.real_speed_bytes_mb ||=5;cfg.real_speed_top_n ||=10;
...
function drawScanDetail(){
  ...
  const rowHtml=(r,kind)=>`<div class="result-row ${kind}"><span class="ip">${esc(r.ip)}</span><span>${esc(r.colo||(!s.cfst?.colo?.length?'不限地区':'未识别'))}</span><span>${Number(r.latency_ms)>0?'TCP '+fmtNum(r.latency_ms,0)+'ms':'—'}${r.real_tested?`<small>真 ${fmtNum(r.real_latency_ms,0)}ms${Number(r.real_first_ms)>0&&Math.abs(Number(r.real_first_ms)-Number(r.real_latency_ms))>20?` · 首连 ${fmtNum(r.real_first_ms,0)}ms`:''}</small>`:''}</span><span>${r.speed_tested?'直 '+fmtNum(r.speed_mb,1)+'M':(r.reject_reason?esc(compactError(r.reject_reason)):'待测速')}${r.real_speed_tested?`<small>真 ${fmtNum(r.real_speed_mb,1)}M</small>`:''}</span></div>`;
  ...
  const speedHint=phase==='speed'?'<div class="speed-hint">速度决赛采用单路测速，避免多个 IP 同时争抢本机带宽导致结果失真；候选越多，这一阶段越慢。</div>':'';
...
function parseVlessLocal(raw){
  raw=String(raw||'').trim();
  if(!raw)throw new Error('节点链接为空');
  let u;try{u=new URL(raw)}catch(e){throw new Error('VLESS 链接格式无效')}
  if(String(u.protocol||'').toLowerCase()!=='vless:')throw new Error(`当前仅支持 VLESS，收到 ${u.protocol||'未知协议'}`);
  const uuid=decodeURIComponent(String(u.username||'').trim());
  const port=Number(u.port||0);
  if(!uuid)throw new Error('VLESS 缺少 UUID');
  if(!Number.isInteger(port)||port<1||port>65535)throw new Error('VLESS 端口无效');
  const q=u.searchParams;
  const ech=q.get('ech')||'';
  let echName='',echDoH='';
  if(ech){
    const parts=ech.split('+');
    echName=(parts.shift()||'').trim();
    echDoH=parts.join('+').trim();
  }
  const network=String(q.get('type')||'ws').toLowerCase();
  if(network!=='ws')throw new Error(`当前 v0.7 真连接核心先支持 VLESS + WebSocket；该节点 type=${network}`);
  const security=String(q.get('security')||'tls').toLowerCase();
  const host=q.get('host')||'';
  const sni=q.get('sni')||host;
  if(security==='tls'&&!sni&&!host)throw new Error('TLS 节点需要 SNI 或 Host');
  const truthy=v=>['1','true','yes','on'].includes(String(v||'').toLowerCase());
  return {
    id:'',
    name:decodeURIComponent(u.hash.replace(/^#/,''))||'VLESS 节点',
    protocol:'vless',
    server:u.hostname,
    port,
    uuid,
...
function renderSettings(){
  ...
  <div class="settings-card"><h3>真连接 / 真测速</h3><div class="modal-note"><b>${buildInfo.real_test_ready?'sing-box 核心已就绪':'当前镜像没有 sing-box 核心'}</b>。真连接延迟按 v2rayN 口径：同一 SOCKS5 客户端连续请求 2 次并取最小值；连接允许复用。页面同时保留“首连”诊断值。</div>
    ...
    <div class="grid3"><label class="field"><span>真连接次数</span><input id="setRealAttempts" type="number" min="1" max="5" value="${Number(cfg.real_test_attempts||2)}"></label><label class="field"><span>真连接并发</span><input id="setRealConcurrency" type="number" min="1" max="16" value="${Number(cfg.real_test_concurrency||5)}"><small>默认 5；整批候选共用一个 sing-box。</small></label><label class="field"><span>真测速每 IP 上限 / MB</span><input id="setRealSpeedBytes" type="number" min="1" max="100" value="${Number(cfg.real_speed_bytes_mb||5)}"></label><label class="field"><span>真测速最多候选</span><input id="setRealSpeedTopN" type="number" min="1" max="100" value="${Number(cfg.real_speed_top_n||10)}"></label></div>
    <label class="field"><span>真测速地址</span><input id="setRealSpeedURL" value="${esc(cfg.real_speed_url||cfg.speed_url||DEFAULT_SPEED_URL)}"><small>只有线路开启“真测速”才使用。v0.7.5 起真测速与真延迟统一使用 SOCKS5，并先做 256KB 热身再计时，避免 5MB 小样本被首次 VLESS/ECH/TLS/WS 建连时间严重拉低。设置“真速度≥”后仍是硬门槛。</small></label>
  ...
  $('#parseRealUri').onclick=()=>{const uri=$('#realUriPaste').value.trim();if(!uri)return toast('先粘贴 VLESS 链接',true);try{const p=parseVlessLocal(uri);p.id=uid('real');openRealProfileEditor(p)}catch(e){toast(`识别失败：${e.message}`,true)}};
  ...
  $('#saveGlobal').onclick=async()=>{cfg.probe_url=$('#setProbeURL').value.trim()||DEFAULT_PROBE_URL;cfg.speed_url=$('#setSpeedURL').value.trim()||DEFAULT_SPEED_URL;cfg.real_test_url=$('#setRealTestURL').value.trim()||DEFAULT_REAL_TEST_URL;cfg.real_test_attempts=Math.max(1,Math.min(5,Number($('#setRealAttempts').value)||2));cfg.real_test_concurrency=Math.max(1,Math.min(16,Number($('#setRealConcurrency').value)||5));cfg.real_speed_url=$('#setRealSpeedURL').value.trim()||cfg.speed_url;cfg.real_speed_bytes_mb=Math.max(1,Math.min(100,Number($('#setRealSpeedBytes').value)||5));cfg.real_speed_top_n=Math.max(1,Math.min(100,Number($('#setRealSpeedTopN').value)||10));cfg.health_check_minutes=Math.max(1,Number($('#setHealth').value)||60);cfg.max_sample_count=Math.max(1,Math.min(100000,Number($('#setMaxSample').value)||10000));cfg.furnace_retention_days=Math.max(7,Number($('#setRetention').value)||45);cfg.furnace_auto_rank=$('#setFurnaceRank').checked;cfg.max_concurrency=Math.max(1,Number($('#setConcurrency').value)||2);cfg.listen=$('#setListen')?.value.trim()||cfg.listen||':8080';cfg.sources.forEach(s=>s.sample_count=Math.min(Number(s.sample_count||256),cfg.max_sample_count));await saveConfig('全局设置已保存')};
...
function analyzeInputsUI(inputs,family){
  ...
  for(const raw of inputs){
    ...
    seen.add(x.value);normalized.push(x.value);
    if(x.remote)remote++;
    if(x.fixed)fixed++;
    if(x.range)ranges++;
  }
...
function capacityText(info,req){
  ...
  const parts=[];
  const fixed=Math.max(0,Number(info.fixed||0));
  const challenge=Math.max(0,req-fixed);
  if(fixed)parts.push(`<b>★ 固定种子 ${fixed}</b>（每轮必测）`);
  if(info.ranges)parts.push(`候选池 ${info.ranges} 个 CIDR`);
  if(info.remote)parts.push(`远程池 ${info.remote} 个`);
  if(info.decorated)parts.push(`${info.decorated} 行自动清洗`);
  if(info.huge)parts.push('IPv6 巨大地址池');
  else if(info.unknown)parts.push('远程容量运行时确认');
  else parts.push(`总可扫描容量 ${info.capacity.toString()}`);
  if(fixed)parts.push(`本轮 ${req} = 种子 ${fixed} + 挑战抽样约 ${challenge}`);
  else parts.push(`本轮 ${req}`);
  return parts.join(' · ');
}
