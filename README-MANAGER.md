...
`data/config.example.json` 预置了四个可删除/可重命名的池：
...
1. 把 首次启动后编辑自动生成的 `data/config.json`，填写 Cloudflare Zone ID / API Token。
2. 把四个示例 Source 的 `inputs` 改成你自己的 IP 段 URL / 文件 / CIDR；也可以直接在 Web 中修改。
3. 启动：
...
- 测速失败时保留上一次成功结果。
- Target 引用某 Source 并要求 N 个 IP 时，如果当前有效结果不足 N 个，本次 DNS 同步直接拒绝，不删除线上旧记录。
- `max_concurrency` 限制全局同时运行的测速任务数（修改后重启容器生效）。
